...
void ListNode::setCur (pptr<ListNode> ptr) { this->curPtr = ptr; }

void ListNode::setNext (pptr<ListNode> ptr) { this->nextPtr = ptr; }

void ListNode::setPrev (pptr<ListNode> ptr) { this->prevPtr = ptr; }

void ListNode::setMin (Key_t key) { this->min = key; }

void ListNode::setMax (Key_t key) { this->max = key; }
...
void ListNode::setDeleted (bool deleted) { this->deleted = deleted; }
...
pptr<ListNode> ListNode::getNextPtr () { return nextPtr; }

pptr<ListNode> ListNode::getCurPtr () { return curPtr; }
...
pptr<ListNode> ListNode::getPrevPtr () { return prevPtr; }

version_t ListNode::readLock (uint64_t genId) { return verLock.read_lock (genId); }

version_t ListNode::writeLock (uint64_t genId) { return verLock.write_lock (genId); }

bool ListNode::readUnlock (version_t oldVersion) { return verLock.read_unlock (oldVersion); }

void ListNode::writeUnlock () { return verLock.write_unlock (); }
...
std::pair<Key_t, Val_t>* ListNode ::getKeyArray () { return this->keyArray; }

std::pair<Key_t, Val_t>* ListNode ::getValueArray () { return this->keyArray; }

Key_t ListNode ::getMin () { return this->min; }

Key_t ListNode::getMax () { return this->max; }

bool ListNode::getDeleted () { return deleted; }

bool compare2 (std::pair<Key_t, uint8_t>& x, std::pair<Key_t, uint8_t>& y) {
    return x.first < y.first;
}
bool compare (std::pair<Key_t, Val_t>& i, std::pair<Key_t, Val_t>& j) { return i.first < j.first; }
...
pptr<ListNode> ListNode ::split (Key_t key, Val_t val, uint8_t keyHash, int threadId, void** olog) {
    ...
    Oplog::writeOpLog (oplog, OpStruct::insert, newMin, (void*)curPtr.getRawPtr (), poolId, key,
                       val);
...
ListNode* ListNode ::mergeWithNext (uint64_t genId) {
    ...
    Oplog::writeOpLog (oplog, OpStruct::remove, getMin (), (void*)getNextPtr ().getRawPtr (),
                       poolId, -1, -1);
...
ListNode* ListNode ::mergeWithPrev (uint64_t genId) {
    ...
    Oplog::writeOpLog (oplog, OpStruct::remove, mergeNode->getMin (),
                       (void*)mergeNode->getNextPtr ().getRawPtr (), poolId, -1, -1);
    oplog->newNodeOid = pmemobj_oid (deleteNode);
    flushToNVM ((char*)oplog, sizeof (OpStruct));
    smp_wmb ();
    // printf("case 6\n");
...
bool ListNode::insert (Key_t key, Val_t value, int threadId) {
    ...
    if (!insertAtIndex (std::make_pair (key, value), (uint8_t)index, keyHash, true)) return false;
    return true;
}
#ifdef SYNC
int ListNode::insert (Key_t key, Val_t value, void** locked_parent_node) {
    ...
    if (index == -2) {              // No free index
        ...
        void* nNode = reinterpret_cast<void*> (((unsigned long)(oplog->poolId)) << 48 |
                                               (oplog->newNodeOid.off));
...
bool ListNode ::checkRange (Key_t key) { return min <= key && key < max; }
...
#ifdef STRINGKEY
    for (int i = 0; i < MAX_ENTRIES; i++) {
        if (bitMap[i]) printf ("%s, ", keyArray[i].first);
    }
#else
    for (int i = 0; i < MAX_ENTRIES; i++) {
        if (bitMap[i]) printf ("%lu, ", keyArray[i].first);
    }
#endif
    std::cout << "::" << std::endl;
}

bool ListNode::scan (Key_t startKey, int range, std::vector<Val_t>& rangeVector,
                     uint64_t writeVersion, uint64_t genId) {
restart:
    ListNode* next = nextPtr.getVaddr ();
...
hydra::bitset* ListNode::getBitMap () { return &bitMap; }

uint8_t* ListNode::getFingerPrintArray () { return fingerPrint; }
