...
void zeroMemAmount () { pmemAmount.store (0); }

void addMemAmount (unsigned long amt) { pmemAmount += amt; }
void printMemAmount () { printf ("amount :%lu\n", pmemAmount.load ()); }
...
restart:
    ...
    if (!cur->writeLock (genId)) goto restart;
    if (cur->getDeleted ()) {
        cur->writeUnlock ();
        goto restart;
    }
...
uint64_t LinkedList::scan (Key_t startKey, int range, std::vector<Val_t>& rangeVector,
                           ListNode* head) {
restart:
    ListNode* cur = head;
    rangeVector.clear ();
    ...
    while (rangeVector.size () < range && !end) {
        ...
        /*resultBuffer.clear();*/
        if (cur->getDeleted ()) goto restart;
