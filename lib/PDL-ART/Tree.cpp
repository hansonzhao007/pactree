...
restart:
    ...
    while (true) {
        ...
        if (N::isLeaf (nextNode)) {
            ...
            level++;
            assert (level <
                    key.getKeyLen ());  // prevent inserting when prefix of key exists already
            ...
            N4* n4 =
                (N4*)new (n4Ptr.getVaddr ()) N4 (level + prefixLength, &k[level], prefixLength);
            pptr<N> pTid (0, (unsigned long)N::setLeaf (tid));
...
typename Tree::CheckPrefixPessimisticResult Tree::checkPrefixPessimistic (N* n, const Key& k,
                                                                          uint32_t& level,
                                                                          uint8_t& nonMatchingKey,
...
TID Tree::lookupNext (const Key& start, ThreadInfo& threadEpocheInfo) const {
    ...
    std::function<void (const N*)> copyReverse = [&result, &resultSize, &resultsFound, &toContinue,
                                                  &copyReverse] (const N* node) {
        if (N::isLeaf (node)) {
            if (resultsFound == resultSize) {
                toContinue = N::getLeaf (node);
                return;
    ...
    std::function<void (N*, N*, uint32_t, uint32_t)> findStart =
        [&copy, &copyReverse, &start, &findStart, &toContinue, &restart, this] (
...
#ifdef SYNC
TID Tree::lookupNextwithLock (const Key& start, void** savenode,
    ...
    TID result[resultSize];
    uint64_t gId = genId;
    std::function<void (N*)> copy = [&result, &resultSize, &resultsFound, &toContinue, &restart,
                                     &gId, &savenode, &copy] (N* node) {
        // std::function<void(const N *)> copy = [&result, &resultSize, &resultsFound,
        // &toContinue,&restart, &gId, &savenode, &copy](const N *node) {
        if (N::isLeaf (node)) {
            if (resultsFound == resultSize) {
                toContinue = N::getLeaf (node);
                return;
    ...
    std::function<void (N*)> copyReverse = [&result, &resultSize, &resultsFound, &toContinue,
                                            &restart, &gId, &savenode, &copyReverse] (N* node) {
        // std::function<void(const N *)> copyReverse = [&result, &resultSize, &resultsFound,
        // &toContinue, &restart,&gId,&savenode,&copyReverse](const N *node) {
        if (N::isLeaf (node)) {
            if (resultsFound == resultSize) {
                toContinue = N::getLeaf (node);
                return;
