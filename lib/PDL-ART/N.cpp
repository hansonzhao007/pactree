...
void N::setLevel (uint32_t lev) { level = lev; }
...
void N::insertGrow (curN* n, N* parentNode, uint8_t keyParent, uint8_t key, pptr<N> val,
...
void N::insertCompact (curN* n, N* parentNode, uint8_t keyParent, uint8_t key, pptr<N> val,
                       ThreadInfo& threadInfo, bool& needRestart, OpStruct* oplog, uint32_t genId) {
#ifdef MULTIPOOL
    int chip, core;
    read_coreid_rdtscp (&chip, &core);
...
void N::insertAndUnlock (N* node, N* parentNode, uint8_t keyParent, uint8_t key, pptr<N> val,
                         ThreadInfo& threadInfo, bool& needRestart, OpStruct* oplog,
                         uint32_t genId) {
    switch (node->getType ()) {
        case NTypes::N4: {
            auto n = static_cast<N4*> (node);
            ...
            insertGrow<N4, N16> (n, parentNode, keyParent, key, val, threadInfo, needRestart, oplog,
                                 genId);
            break;
        }
        case NTypes::N16: {
...
void N::removeAndShrink (curN* n, N* parentNode, uint8_t keyParent, uint8_t key,
    ...
    smallerN* nSmall =
        (smallerN*)new (nSmallPtr.getVaddr ()) smallerN (n->getLevel (), n->getPrefi ());
...
void N::removeAndUnlock (N* node, uint8_t key, N* parentNode, uint8_t keyParent,
    ...
    switch (node->getType ()) {
        ...
        case NTypes::N16: {
            ...
            removeAndShrink<N16, N4> (n, parentNode, keyParent, key, threadInfo, needRestart, oplog,
                                      genId);
            break;
        }
        case NTypes::N48: {
...
bool N::checkOrRestart (uint64_t startRead) const { return readUnlockOrRestart (startRead); }
...
void N::setPrefix (const uint8_t* prefix, uint32_t length, bool flush) {
    ...
    if (flush) flushToNVM ((char*)&(this->prefix), sizeof (Prefix));
}
