...
public:
    ...
    StringKey (const StringKey& other) { memcpy (data, other.data, keySize); }
    StringKey (const char bytes[]) { set (bytes, strlen (bytes)); }
    StringKey (int k) { setFromString (std::to_string (k)); }
...
public:
    ...
    uint64_t ts;         // 8
    bool operator< (const OpStruct& ops) const { return (ts < ops.ts); }

    void Reset () { memset (this, 0, 64); }
};

static_assert (sizeof (OpStruct) == 64);
#endif
