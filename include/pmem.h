...
public:
    static void* getBaseOf (int poolId) { return baseAddresses[poolId]; }
    ...
    // case 1
    static bool bind (int poolId, const char* nvm_path, size_t size, void** root_p,
                      int* is_created) {
        ...
        if (access (nvm_path, F_OK) != 0) {
            ...
            if (pop == nullptr) {
                std::cerr << "bind create error "
                          << "path : " << nvm_path << std::endl;
                return false;
            }
            printf ("create bind poolId: %d, %s\n", poolId, nvm_path);
